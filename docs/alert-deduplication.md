...
2. **Commit (publish success).** `nats.PublishEvents` invokes an `onPublished` callback for each event that NATS accepted. The alert handler commits the fingerprint and change time there (`UpdateIfNewer`). Failed publishes commit nothing.
